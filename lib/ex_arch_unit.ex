defmodule ExArchUnit do
  ...
  defmacro __using__(opts) do
    config_path = Keyword.get(opts, :config, ExArchUnit.Config.default_config_path())
    ...
    quote bind_quoted: [config_path: config_path, enforce_config_rules: enforce_config_rules] do
      import ExArchUnit, only: [forbid: 2, allow: 2, assert_no_cycles: 1]
  ...
  defmacro forbid(source_selector, opts) do
    config_path =
      Module.get_attribute(__CALLER__.module, :ex_arch_config_path) ||
        ExArchUnit.Config.default_config_path()

    quote bind_quoted: [source_selector: source_selector, opts: opts, config_path: config_path] do
      ExArchUnit.__assert_forbid__(binding(), source_selector, opts, config_path)
    end
  end
  ...
  *not* matching the `:depends_on` target selector. Dependencies within
  ...
  defmacro allow(source_selector, opts) do
    ...
    quote bind_quoted: [source_selector: source_selector, opts: opts, config_path: config_path] do
      ExArchUnit.__assert_allow__(binding(), source_selector, opts, config_path)
    end
  end
  ...
  defmacro assert_no_cycles(opts) do
    ...
    quote bind_quoted: [opts: opts, config_path: config_path] do
      ExArchUnit.__assert_no_cycles__(binding(), opts, config_path)
    end
  end
  ...
  def __assert_forbid__(binding, source_selector, opts, config_path) do
    ...
    case ExArchUnit.Rule.Evaluator.forbid(graph, config, source_selector, target_selector) do
      :ok ->
        :ok

      {:error, violations} ->
        ExUnit.Assertions.flunk(
          ExArchUnit.Reporter.format_forbid(source_selector, target_selector, violations)
        )
    end
  end
  ...
  def __assert_allow__(binding, source_selector, opts, config_path) do
    ...
    case ExArchUnit.Rule.Evaluator.allow(graph, config, source_selector, target_selector) do
      :ok ->
        :ok

      {:error, violations} ->
        ExUnit.Assertions.flunk(
          ExArchUnit.Reporter.format_allow(source_selector, target_selector, violations)
        )
    end
  end
  ...
  def __assert_no_cycles__(binding, opts, config_path) do
    ...
    case ExArchUnit.Rule.Evaluator.assert_no_cycles(graph, config, opts) do
      :ok ->
        :ok

      {:error, cycles} ->
        ExUnit.Assertions.flunk(ExArchUnit.Reporter.format_cycles(selector, cycles))
    end
  end

  @doc false
  def __assert_config_rules__(graph, config) do
    case ExArchUnit.Rule.Evaluator.evaluate_layer_rules(graph, config) do
      :ok ->
        :ok

      {:error, layer_rule_violations} ->
        ExUnit.Assertions.flunk(ExArchUnit.Reporter.format_layer_rules(layer_rule_violations))
    end
  end
  ...
  defp resolve_context(binding, config_path) do
    ...
    case context do
      ...
      _ ->
        config = ExArchUnit.Config.load!(config_path)
        {graph, _stats} = ExArchUnit.Graph.Cache.get_or_build(config)
        {graph, config}
    end
  end
end
