apps =
  System.get_env("ExArch_BENCH_APPS", "4")
  |> String.to_integer()

modules_per_app =
  System.get_env("ExArch_BENCH_MODULES_PER_APP", "60")
  |> String.to_integer()

cleanup? =
  System.get_env("ExArch_BENCH_KEEP_TMP", "0")
  |> String.downcase()
  |> then(&(&1 not in ["1", "true", "yes"]))

result =
  ExArch.Benchmark.SyntheticUmbrella.run(
    apps: apps,
    modules_per_app: modules_per_app,
    cleanup: cleanup?
  )

IO.puts("ex_arch synthetic umbrella benchmark")
